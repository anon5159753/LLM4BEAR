...
class PromptGenerator(object):  
    ...
    def get_test_prompts(self, data_info):
        ...
        for idx, item_title in enumerate(data_info.split('|split|')):
            idx_item = "product" + str(idx+1)
            test_item_titles[idx_item] = item_title
        test_prompts += str(test_item_titles) + '\n'
        test_prompts += """Try to infer any intents that may exist in the list of products if purchased together, each intent should help form a bundle with associated products.
...
"""
        ...
        # ", the answer format is: {'bundle number':['product number', 'product number']}. Note that each bundle MUST contain two or more products. Use ONLY the product numbers. DO NOT use the item titles. No explanation for the results."
